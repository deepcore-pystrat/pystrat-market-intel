...
from pystrat.features.features_services import zscore_ewm_smooth
...
_LOWS = [-i / 10.0 for i in range(10, 21, 2)]
_HIGHS = [i / 10.0 for i in range(10, 21, 2)]


def add_features(context: Context) -> Context:
    """Add the causal z-score-of-spread column (VHP - THP, no shift)."""
    frames = dict(context.frames)
    frame = frames[SB11_KEY]
    spread = frame[BASIS_VHP_COLUMN] - frame[BASIS_THP_COLUMN]
    signal = zscore_ewm_smooth(spread, window=_ZSCORE_WINDOW, smooth_span=_SMOOTH_SPAN)
...
def build_strategy(params: Mapping[str, Any]) -> Strategy:
    """Reversion cross on the z-score, fixed-notional SB11 target."""
...
def decision_series(context: Context, schedule : Mapping[str, Any]) -> Mapping[str, Any]:
    """Panel-2 series for the diagnostic: the causal z-score of the spread."""
    return {"zscore": context.frames[SB11_KEY][SIGNAL_COLUMN]}
...
def profile() -> MarketIntelProfile:
    ...
    return MarketIntelProfile(
        ...
        optimizer=OptunaOptimizer.from_search_space(make_space(), n_trials=25, seed=42),
